...
{
    public partial class LoanSettlementForeclosureReport : System.Web.UI.Page
    {
        ...
        private void LoadReport()
        {
            ...
            using (SqlCommand cmd = new SqlCommand("usp_GetSettlementForeclosureDetails", con))
            {
                cmd.CommandType = CommandType.StoredProcedure;

                cmd.Parameters.AddWithValue("@SettlementType", ddlSettlementType.SelectedValue);
        ...
        private void CalculateKPI(DataTable dt)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                if (row["FineAmount"] != DBNull.Value)
                    totalFine += Convert.ToDecimal(row["FineAmount"]);
            }
